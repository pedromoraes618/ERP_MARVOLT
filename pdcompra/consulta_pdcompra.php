...
if($_GET){
    ...
    $statusEntrega = $_GET["status_entrega"];
    ...
    $pesquisaDataf = $div2[2]."-".$div2[1]."-".$div2[0];
        ...
        if($statusEntrega == 1){
            ...
            clientes.nome_fantasia LIKE '%{$pesquisa}%' and pedido_compra.numero_pedido_compra LIKE '%{$pesquisaNpedido}%' and pedido_compra.entrega_realizada !='0000-00-00' " ;
...
'Titulo da Janela', 'STATUS=NO, TOOLBAR=NO, LOCATION=NO, DIRECTORIES=NO, RESISABLE=NO, SCROLLBARS=YES, TOP=10, LEFT=10, WIDTH=1600, HEIGHT=900');">
            ...
            <form style="width:100%;" action="" method="get">
                ...
                <select style="width: 150px; float:right; margin-right:100px;" id="status" name="status_entrega">
                    <option value="0">Status de entrega</option>
                    <option value="1" <?php if(isset($_POST)){
                        if($statusEntrega == 1){
                            ?>selected <?php
                        }
                    } ?>>Entregue</option>
                    <option value="2" <?php if(isset($_POST)){
                        if($statusEntrega == 2){
                            ?>selected <?php
                        }
                    } ?>>Não entregue</option>
