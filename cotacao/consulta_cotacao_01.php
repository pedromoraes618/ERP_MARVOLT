...
//consultar cotacao
if(isset($_GET["CampoPesquisaData"]) && isset($_GET['CampoPesquisaDataf']) )  {
    ...
    $clienteID = $_GET["campoCliente"];
    $situacao = $_GET['campoSituacao'];
    $pesquisaNorcamento = $_GET["CampoPesquisaNorcamento"];
    
    if($situacao == 1 and $clienteID == 0 ){
        $select .= " WHERE data_lancamento BETWEEN '$pesquisaData' and '$pesquisaDataf'  and cotacao.numero_orcamento  LIKE '%{$pesquisaNorcamento}%' order by cotacao.numero_orcamento  ";
    }elseif($situacao == 1 and $clienteID != 0 ){
        $select .= " WHERE data_lancamento BETWEEN '$pesquisaData' and '$pesquisaDataf'  and cotacao.clienteID = $clienteID and cotacao.numero_orcamento  LIKE '%{$pesquisaNorcamento}%' order by cotacao.numero_orcamento  ";
    }else{
        $select .= " WHERE data_lancamento BETWEEN '$pesquisaData' and '$pesquisaDataf' and cotacao.clienteID = $clienteID and cotacao.numero_orcamento  LIKE '%{$pesquisaNorcamento}%' and cotacao.status_proposta LIKE '%{$situacao}%' order by cotacao.numero_orcamento  ";
    }
...
'Cadastro_cotacao', 'STATUS=NO, TOOLBAR=NO, LOCATION=NO, DIRECTORIES=NO, RESISABLE=NO, SCROLLBARS=YES, TOP=10, LEFT=10, WIDTH=1600, HEIGHT=900');">
                ...
                <select style="margin:0 auto;  width:400px;" name="campoCliente" id="campoCliente">
