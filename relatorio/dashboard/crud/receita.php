...
clt.cpfcnpj = nfe.cnpj_cpf inner join tb_grupo_cliente as gpc on gpc.cl_id = clt.grupo_cliente and nfe.data_emissao BETWEEN '$ano-$mes_ini-01' and '$ano-$mes_fim-31' and nfe.status_processamento ='1' and clt.estadoID= '10'";
...
clt.cpfcnpj = nfe.cnpj_cpf inner join tb_grupo_cliente as gpc on gpc.cl_id = clt.grupo_cliente and nfe.data_emissao BETWEEN '$ano-$mes_ini-01' and '$ano-$mes_fim-31' and nfe.status_processamento ='1'";
...
if(isset($_GET['id_grupo_cliente_bloco_left'])){
    ...
    $select = "SELECT SUM(lcf.valor) as valor,lcf.clienteID,cl.nome_fantasia as cliente,gpcl.cl_id as id, gpcl.cl_descricao as grupo from lancamento_financeiro as lcf 
    inner join clientes as cl on cl.clienteID = lcf.clienteID inner join tb_grupo_cliente as gpcl on gpcl.cl_id= cl.grupo_cliente  where 
    lcf.receita_despesa = 'Receita' and status='Recebido' and lcf.data_do_pagamento 
    BETWEEN '$ano-$mes_ini-01' and '$ano-$mes_fim-31' and gpcl.cl_id = '$id_grupo_cliente' group by cl.cpfcnpj ";
    $consulta_clientes_agrupado_por_cnpj = mysqli_query($conecta,$select);

    /*somar os valores do cliente por nota fiscal */
    $select = "SELECT SUM(lcf.valor) as somatorio,lcf.clienteID,cl.nome_fantasia as cliente,gpcl.cl_id as id, gpcl.cl_descricao as grupo from lancamento_financeiro as lcf 
    inner join clientes as cl on cl.clienteID = lcf.clienteID inner join tb_grupo_cliente as gpcl on gpcl.cl_id= cl.grupo_cliente  where 
    lcf.receita_despesa = 'Receita' and status='Recebido' and lcf.data_do_pagamento 
