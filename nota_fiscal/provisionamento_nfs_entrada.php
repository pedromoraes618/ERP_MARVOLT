...
$nNfe =  $_GET["codigo"];
$id_referencia =  $_GET["referencia"];
}

$consulta = "SELECT * FROM tb_nfs_entrada";
$consulta .= " WHERE numero_nf = {$nNfe} and id_referencia = {$id_referencia}  ";  
$dados_pedido= mysqli_query($conecta, $consulta);
if(!$dados_pedido){
die("Falaha no banco de dados");
}else{
    $linha = mysqli_fetch_assoc($dados_pedido);
    $cnpj = $linha['cnpj_prestador'];
    $valorContabilNf = $linha['vTotal_servico'];
}
...
if(isset($_POST['salvar'])){
    ...
    $mensagem = utf8_decode("Duplicata $documento referente a nota fiscal de serviço de entrada $nNfe");
    $salvar = $_POST['salvar'];
    
    $select = "SELECT sum(valor) as total from lancamento_financeiro WHERE numeroNotaFiscal = '$nNfe' and id_referencia = $id_referencia and modalidade = 'NFS_ENTRADA'";
        ...
        <?php
            ...
            if( $_SERVER['REQUEST_METHOD']=='POST' )
            {
                ...
                else
                {
                    ...
                    $inserir .= "( data_movimento,data_a_pagar,receita_despesa,status,forma_pagamentoID,clienteID,descricao,documento,grupoID,valor,numeroNotaFiscal,id_referencia,modalidade,cl_conta_financeira_id )";
                    $inserir .= " VALUES ";
                    $inserir .= "( '$hoje','$dataApagar','Despesa','A Pagar','$formaPagamento','$cliente','$mensagem','$documento','17','$valor','$nNfe','$id_referencia','NFS_ENTRADA','$conta_financeira' )";
...
if(isset($_POST['salvar'])or (isset($_GET['codigo']))){
    $select = "SELECT * from lancamento_financeiro WHERE numeroNotaFiscal = '$nNfe' and id_referencia = $id_referencia and modalidade = 'NFS_ENTRADA'";
                            ...
                            <tr>
                                <td>
                                    ...
                                    <select style="margin-right: 60px; margin-bottom:10px; width:580px;"
                                        ...
                                        $meuCliente = $cnpj; // comparando com o fornecedor
